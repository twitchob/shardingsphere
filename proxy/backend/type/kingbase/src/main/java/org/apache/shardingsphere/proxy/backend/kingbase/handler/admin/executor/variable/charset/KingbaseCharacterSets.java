...
package org.apache.shardingsphere.proxy.backend.kingbase.handler.admin.executor.variable.charset;
 ...
 */
public enum KingbaseCharacterSets {
    ...
    private static final Map<String, KingbaseCharacterSets> CHARACTER_SETS_MAP;
    
    static {
        Map<String, KingbaseCharacterSets> map = new HashMap<>(128, 1F);
        for (KingbaseCharacterSets each : values()) {
    ...
    KingbaseCharacterSets(final Supplier<Charset> charsetSupplier, final String... aliases) {
        Charset result = null;
        try {
    ...
    public static Charset findCharacterSet(final String charsetName) {
        ...
        KingbaseCharacterSets result = CHARACTER_SETS_MAP.get(formattedCharsetName.toUpperCase());
