...
import org.apache.shardingsphere.db.protocol.kingbase.packet.command.PostgreSQLCommandPacketFactory;
...
import org.apache.shardingsphere.proxy.frontend.kingbase.err.KingbaseErrorPacketFactory;
 ...
 */
public final class KingbaseCommandExecuteEngine implements CommandExecuteEngine {
    ...
    public CommandExecutor getCommandExecutor(final CommandPacketType type, final CommandPacket packet, final ConnectionSession connectionSession) throws SQLException {
        PortalContext portalContext = KingbasePortalContextRegistry.getInstance().get(connectionSession.getConnectionId());
        return KingbaseCommandExecutorFactory.newInstance((PostgreSQLCommandPacketType) type, (PostgreSQLCommandPacket) packet, connectionSession, portalContext);
    }
    
    @Override
    public PostgreSQLPacket getErrorPacket(final Exception cause) {
        return KingbaseErrorPacketFactory.newInstance(cause);
    }
    ...
    public void writeQueryData(final ChannelHandlerContext context,
        ...
        if (ResponseType.QUERY == queryCommandExecutor.getResponseType() && !context.channel().isActive()) {
            context.write(new PostgreSQLCommandCompletePacket(KingbaseCommand.SELECT.name(), 0L));
            return;
        }
    ...
    private void processSimpleQuery(final ChannelHandlerContext context, final ProxyDatabaseConnectionManager databaseConnectionManager,
        ...
        if (ResponseType.QUERY == queryExecutor.getResponseType()) {
            context.write(new PostgreSQLCommandCompletePacket(KingbaseCommand.SELECT.name(), dataRows));
        }
        context.write(databaseConnectionManager.getConnectionSession().getTransactionStatus().isInTransaction() ? PostgreSQLReadyForQueryPacket.IN_TRANSACTION
                : PostgreSQLReadyForQueryPacket.NOT_IN_TRANSACTION);
    }
