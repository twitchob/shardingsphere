...
/**
 * Aggregated batched statements command executor for Kingbase.
 */
@RequiredArgsConstructor
public final class KingbaseAggregatedBatchedStatementsCommandExecutor implements CommandExecutor {
    ...
    public Collection<DatabasePacket> execute() throws SQLException {
        KingbaseServerPreparedStatement preparedStatement = getPreparedStatement();
        KingbaseBatchedStatementsExecutor executor = new KingbaseBatchedStatementsExecutor(connectionSession, preparedStatement, readParameterSets(preparedStatement.getParameterTypes()));
        ...
        for (PostgreSQLCommandPacket each : packets) {
            ...
            if (each instanceof PostgreSQLComExecutePacket) {
                String tag = KingbaseCommand.valueOf(preparedStatement.getSqlStatementContext().getSqlStatement().getClass()).orElse(KingbaseCommand.INSERT).getTag();
    ...
    private KingbaseServerPreparedStatement getPreparedStatement() {
