...
package org.apache.shardingsphere.proxy.frontend.kingbase.command.query;
...
public enum KingbaseCommand {
    ...
    @SafeVarargs
    KingbaseCommand(final Class<? extends SQLStatement>... sqlStatementClasses) {
     ...
     */
    public static Optional<KingbaseCommand> valueOf(final Class<? extends SQLStatement> sqlStatementClass) {
     ...
     */
    private static Optional<KingbaseCommand> getPostgreSQLCommand(final Class<? extends SQLStatement> sqlStatementClass) {
        CachedResult result = COMPUTED_STATEMENTS.get(sqlStatementClass);
        return null == result ? COMPUTED_STATEMENTS.computeIfAbsent(sqlStatementClass, KingbaseCommand::compute).get() : result.get();
    }
    
    private static CachedResult compute(final Class<? extends SQLStatement> target) {
        Optional<KingbaseCommand> result = Arrays.stream(KingbaseCommand.values()).filter(each -> matches(target, each)).findAny();
        return result.map(CachedResult::new).orElse(CachedResult.EMPTY);
    }
    
    private static boolean matches(final Class<? extends SQLStatement> sqlStatementClass, final KingbaseCommand command) {
    ...
    private static final class CachedResult {
        ...
        private final KingbaseCommand result;
        
        private Optional<KingbaseCommand> get() {
